#!/usr/bin/env sh
# Regenerate icons/ from the artwork in svg/ using the Chromium that ships with
# this environment. No ImageMagick / librsvg / Pillow needed.
#
#   ./tools/make-icons.sh
set -eu
...
"$CHROME" --headless --disable-gpu --no-sandbox --virtual-time-budget=30000 \
  --dump-dom "http://127.0.0.1:$PORT/tools/make-icons.html" 2>/dev/null \
